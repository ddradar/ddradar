...
This component provides a form for users to input or edit their scores for a specific song chart.
...
const { id, chart, score = null } = defineProps<ScoreInputProps>()
...
<template>
  ...
  >
    <div class="space-y-4">
      ...
      <UFormField :label="t('schema.score.flareRank.label')" name="flareRank">
        <USelect v-model="state.flareRank" :items="flareRankOptions" />
      </UFormField>

      <UFormField :label="t('schema.score.flareSkill.label')" name="flareSkill">
        <UInput
